...
const FullDock: React.FC<DockProps> = ({ currentApp = 'bitcoin-chat' }) => {
  ...
  const [isMinimal, setIsMinimal] = useState(true); // Start in minimal state
  ...
  const dockApps: DockApp[] = [
    ...
    { id: 'bitcoin-exchange', name: 'Bitcoin Exchange', icon: TrendingUp, color: 'rainbow', url: '/exchange', current: currentApp === 'bitcoin-exchange' },
    ...
    { id: 'bitcoin-chat', name: 'Bitcoin Chat', icon: MessageCircle, color: 'rainbow', url: '/', current: currentApp === 'bitcoin-chat' },
  ...
  const handleMouseLeave = () => {
    ...
    if (!isMinimal) {
      // Minimize after 500ms of not hovering
      const timeout = setTimeout(() => {
        setIsMinimal(true);
      }, 500);
  ...
  return (
    <>
      {/* Real TickerSidebar from bitcoin-OS */}
      <TickerSidebar userHandle="$User" />

      <div className={`bitcoin-dock ${isMinimal ? 'minimal' : 'full'}`} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
        <div className="dock-container">
          {/* App icons on the left */}
          <div className="dock-apps">
            {(isMinimal ? dockApps.slice(0, 8) : dockApps).map((app, index) => {
          ...
          {isMinimal && (
            <div className="dock-expansion-indicator">
              <span>...</span>
            </div>
          )}
          </div>
